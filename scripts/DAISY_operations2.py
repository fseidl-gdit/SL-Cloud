...
def ProcessGeneAlias (client, input_gene_list, database):
    pancanceratlas_genes_query="""SELECT DISTINCT Gene_Symbol from `pancancer-atlas.Filtered.all_CNVR_data_by_gene_filtered`
    UNION DISTINCT  
    SELECT DISTINCT Symbol from  `pancancer-atlas.Filtered.EBpp_AdjustPANCAN_IlluminaHiSeq_RNASeqV2_genExp_filtered` 
    UNION DISTINCT
    SELECT DISTINCT Hugo_Symbol from `pancancer-atlas.Filtered.MC3_MAF_V5_one_per_tumor_sample`  """
...
def RetrieveSamples(client, data_resource, method, tissues):
    ...
    WHERE SampleType not like '%Normal%' and Study is not null '''
		...
		WHERE SampleType not like '%Normal%'and Study is not null
		INTERSECT DISTINCT
		SELECT distinct SampleBarcode,  Study  FROM `pancancer-atlas.Filtered.all_CNVR_data_by_gene_filtered`
		WHERE SampleType not like '%Normal%' and Study is not null
		INTERSECT DISTINCT
		SELECT distinct Tumor_SampleBarcode as  SampleBarcode,  Study  FROM `pancancer-atlas.Filtered.MC3_MAF_V5_one_per_tumor_sample` WHERE Study is not null)
		TS '''
...
def SurvivalOfFittest(client, SL_or_SDL, data_source, input_genes, percentile_threshold, cn_threshold, adj_method, tissues, input_mutations='None'):
  ...
  sql_without_mutation= '''
    ...
    WHERE GE.__EXP_GENE_NAME__ in (__GENELIST__)  AND __SAMPLE_ID__ in (__SAMPLE_LIST__)
    )
    AS NGE
    ...
    WHERE CN.__CN_GENE_NAME__ in (__GENELIST__)  AND __SAMPLE_ID__ in (__SAMPLE_LIST__)
    ) AS NC
...
def FunctionalExamination(client, SL_or_SDL, database, input_genes, percentile_threshold, cn_threshold, adj_method, tissues,  input_mutations=None):
    ...
    FROM  __GENE_EXP_TABLE__ GE
    WHERE GE.__SYMBOL__ in (__GENELIST__) AND __CCLE_SAMPLE_ID__ in (__SAMPLE_LIST_CCLE__) ) AS NGE
    WHERE NGE.Percentile __GENE_CMP_STR__
    ...
    CN.CNA AS NORM_CN
    FROM  __CN_TABLE__ CN
    WHERE CN.__SYMBOL__ in (__GENELIST__) AND __CCLE_SAMPLE_ID__ in (__SAMPLE_LIST_CCLE__) ) AS NC
    WHERE NC.NORM_CN __CN_CMP_STR__  )"""
